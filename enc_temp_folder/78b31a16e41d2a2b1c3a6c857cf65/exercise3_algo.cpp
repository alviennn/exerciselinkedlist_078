...
void CircularLinkedList::addNode() { //Untuk menambahkan node baru pada list
	...
	if (LAST == NULL) {
		...
		if (ridho == NULL) {
			// Step a1: Allocate memory for the new node
			newnode->next = LAST->next;
			// Step a2: Assign value to the data field of the new node
			LAST->next = newnode;
		}
		else if (alvien == LAST && alvien->rollNumber < newnode->rollNumber) {
			// Step c1: Allocate memory for the new node
			newnode->next = LAST->next;
			// Step c2: Assign value to the data field of the new node
			LAST->next = newnode;
			// Step c5: Mark LAST point to the new node
			LAST = newnode;
		}
		else {
			// Step b4: Make ridho point to alvien
			ridho->next = newnode;
			// Step b5: Make alvien point to the next node in sequence
			newnode->next = alvien;
		}
	}
}
